...
class x265LK:
    ...
    def extract_movie_or_episode_copy(self, tv:bool, link:str) -> dict:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                links_data = download_div.find('tbody').findAll('tr')   # Type : <class 'bs4.element.ResultSet'>                
    ...
    def extract_seasons(self, link: str) -> dict:
        ...
        return a dictionary contains response_code, status_code, data. 
        if not error, data contins list like this:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                for season_html in seasons:
                    ...
                    season_data = season_data_html.find('span', {'class' : 'title'})    # Type : <class 'bs4.element.ResultSet'>
                    ...
                    eps_data_html = season_html.find('div', {'class' : 'se-a'}).find('ul')   # Type : <class 'bs4.element.Tag'>                    
...
# search_result = x265.search(term, tv)
# for i, result in enumerate(search_result['data'], 1):
#     print(i, result)

# n = int(input('No : ').strip()) - 1
# ses = x265.extract_seasons(search_result['data'][n]['url'])


# for i, s in  enumerate(ses['data'], 1):
#     print(i, s['season_no'])

# sn = int(input('No: ').strip()) - 1
# selected_ses = ses['data'][sn]['episodes']


# for i, ep in enumerate(selected_ses, 1):
#     print(i, ep['epi_no'], ep['epi_name'])

# en = int(input('No: ').strip()) - 1
...
# cls = x265.extract_movie_or_episode_copy(False, movie_data=search_result['data'][n])['data']

# for i,cp in enumerate(cls, 1):
#     print(i, cp['copy_name'])

# n = int(input('No: ').strip()) - 1
# sc = cls[n]


# fl = x265.get_download_link(sc['url'])
# print(fl)
